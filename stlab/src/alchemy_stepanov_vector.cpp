...
static AS3_Val bench(void* self, AS3_Val args)
{	
	// output command for documentation:
	int i;
	double dZero = 0.0;
	AS3_ArrayValue(args,"IntType, FloatType", &iterations, &init_value);
	printf("C++ benchmarks 'simple_types_constant_folding (benchmark)' with args : %d - %d\n", iterations, init_value);
...
int main()
{
	AS3_Val benchMethod = AS3_Function(NULL, bench);
	
	AS3_Val result = AS3_Object("bench: AS3ValType", benchMethod);
				
	AS3_Release (benchMethod);
